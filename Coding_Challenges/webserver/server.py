...
class Server:
    ...
    def run_server(self, conn, addr, thread_id):
        ...
        time.sleep(2) # To test multi-threading
        if conn:
            ...
            # Parse request to obtain the GET argument
            ...
            filepath = self.root + filename # Requested file on the server
            ...
            if os.path.exists(filepath):
                ...
                response = self.HTTP_header_success + 'thread id: ' + str(thread_id) + '\n\r\n\r' + content
                conn.sendall(response.encode())
            else:
                conn.sendall(self.HTTP_header_fail.encode())
